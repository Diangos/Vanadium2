 ...
 * @prop {'warning'|'error'} generate - The type of validation
 * @prop {string} message - The error or warning message generated by a validation function.
 * @prop {HTMLElement} - The element for which the error was generated
 */
...
const validator = {
    ...
    /**
     * Handles different result types from validator functions (functions that a string or arrays of strings) etc.
     * WARNING: This function mutates the results array.
     * @param {string|string[]} result - the result of the validation
     * @param {ValidationResult[]} results - The array of results to which we should add the current result
     * @param {Validator} rule - The rule that described the validation for which the error/warning was produced
     * @param {HTMLElement} element - The element for which the validation was done
     * @return {ValidationResult[]}
     * @private
     */
    _addResultToResultsArray(result, results, rule, element) {
        if (result) {
            if (Array.isArray(result)) {
                for (let j = 0; j < result.length; j++) {
                    ...
                    results.push({
                        type: rule.generateWarning ? 'warning' : 'error',
    ...
    init: function (rules) {
        ...
        for (let i = 0; i < rules.length; i++) {
            ...
            // TODO: write and call a rule validator function here
    ...
    validate: function (element, rule) {
        ...
        // Conjunct rules (AND) - all validations must succeed for the input to be considered valid
        outerRule:
        for (let i = 0; i < finalRule.validators.length; i++) {
            const currentOuterRule = finalRule.validators[i];

            if (!Array.isArray(currentOuterRule)) {
                if (!this.utils.validators[currentOuterRule.validator]) {
                    continue;
                }

                const result = this.utils.validators[currentOuterRule.validator](value, currentOuterRule.options, element);

                this._addResultToResultsArray(result, results, currentOuterRule, element);
            } else {
                let innerResults = [];

                // Disjunct rules (OR) - any validation can succeed for the input to be considered valid
                for (let j = 0; j < currentOuterRule.length; j++) {
                    const currentInnerRule = currentOuterRule[j];
                    const result = this.utils.validators[currentInnerRule.validator](value, currentInnerRule.options, element);

                    // If at least one validation is successful we have no more errors to show so we simply move on
                    if (!result) {
                        continue outerRule;
                    }

                    this._addResultToResultsArray(result, innerResults, currentInnerRule, element);
                }

                const tempInnerResultArray = [];

                // Push only the warnings on this temp array since we need to push only warnings to the final results array
                for (let j = 0; j < innerResults.length; j++) {
                    const result = innerResults[j];

                    if (result.generate === 'warning') {
                        tempInnerResultArray.push(result);
                    }
                }

                // If there are no successful validations but there are warnings
                // we simply push the warnings to the results array
                results = tempInnerResultArray.length ? results.concat(tempInnerResultArray) : results.concat(innerResults);
            }
        }
